...
//---------------------------------
jv_mainframe::jv_mainframe(const TGWindow *p, UInt_t w, UInt_t h, bool build_gui) :
...
void jv_mainframe::DoSelectObjectType(Int_t id) {
	...
	cout << "There are: " << objs.size() << " objs " << endl;
	...
	for (uint32_t i = 0; i < objs.size(); i++) {
		...
		sprintf(str, "0x%016lx %s", (unsigned long) (void*)objs[i], ((JObject*) objs[i])->className().c_str());
...
void jv_mainframe::DoSelectObject(Int_t id) {
	...
	Int_t idx = id - 1;
	if (idx < 0 || idx >= (Int_t) objs.size()) return;
	JObject *obj = objs[idx];

	UpdateObjectValues(obj);

	// Get associated objects
	aobjs.clear();
	JEP->GetAssociated(obj, aobjs);
	...
	//cout<<"This object is associated to: "<<a2objs.size()<<" objects "<<endl;
	...
	// Get pointer to selected object as a JObject
	JObjectDrawable *obj2 = 0;
	obj2 = dynamic_cast<JObjectDrawable*>((JObject*) objs[idx]);
	if (obj2 != 0) {
		DrawObject(obj2);
	}
}
...
void jv_mainframe::DrawObject(const JObjectDrawable *obj) {
...
void jv_mainframe::UpdateObjectValues(JObject *obj) {
	...
	JObjectSummary objSum;
	obj->Summarize(objSum);

	vector<pair<string, string> > items;
	for (auto field : objSum.get_fields()) {
		items.push_back(std::make_pair(field.name, field.value));
	}
