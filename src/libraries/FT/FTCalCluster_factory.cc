...
void FTCalCluster_factory::Process(const std::shared_ptr<const JEvent> &aEvent) {
	...
	if (hits.size()<4) return; //Comment before using
	...
	for(auto hit : hits){
		std::cout <<"Hit.x= " <<hit->getHitX() <<", Hit.Y "<< hit->getHitY() <<", Hit.T " <<4*hit->getHitTime().count() <<" nS, Hit.E " <<hit->getHitEnergy()  <<std::endl;
	}
