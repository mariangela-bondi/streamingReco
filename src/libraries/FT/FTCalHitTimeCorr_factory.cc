...
//	CRYS_ZPOS = 1628.;
	...
	daq_gain[1][13][5] = 0.0017934; //Mi lascia perplesso
	...
	daq_gain[1][14][2] = 0.0017934; // Mi lascia perplesso
	...
	/*for (int i = 3; i < 16; i++) {
	 for (int j = 0; j < 16; j++) {
	 cout << "daq_gain[0][" << i << "][" << j << "] = " << daq_gain[0][i][j] << endl;
	 }
	 cout << endl;
	 //		cin.get();
	 }
	 ...
	 for (int i = 3; i < 15; i++) {
	 for (int j = 0; j < 16; j++) {
	 cout << "daq_gain[1][" << i << "][" << j << "] = " << daq_gain[1][i][j] << endl;
	 }
	 cout << endl;
	 //		cin.get();
	 }

	 cout << "CREAZIONE FINITA!" << endl;
	 //	cin.get();*/
...
void FTCalHitTimeCorr_factory::Process(const std::shared_ptr<const JEvent> &aEvent) {
	...
	/*	cout << daq_gain[0][3][0] << " " << daq_gain[0][3][1] << " " << daq_gain[0][3][2] << " " << daq_gain[0][3][3] << " " << daq_gain[0][3][4] << " " << daq_gain[0][3][5] << " " << daq_gain[0][3][6]
	 << " " << daq_gain[0][3][7] << " " << daq_gain[0][3][8] << " " << daq_gain[0][3][9] << " " << daq_gain[0][3][10] << " " << daq_gain[0][3][11] << " " << daq_gain[0][3][12] << " "
	 << daq_gain[0][3][13] << " " << daq_gain[0][3][14] << " " << daq_gain[0][3][15] << endl;
	 cin.get();*/
	...
	for (auto faHit : faHits_fa250VTPMode7) {
		...
		if ((m_channel.det_sys == TranslationTable::FTCAL)) {
			...
			/*			auto charge = faHit->m_charge / daq_gain[theCrate][faHit->m_channel.slot][faHit->m_channel.channel];
			 ...
			 //			cout << "charge = faHit->m_charge / daq_gain[theCrate][faHit->m_channel.slot][faHit->m_channel.channel]; " << charge << endl;
			 ...
			 //			cout << "mips_charge[ftCalHit->m_channel.component] " << mips_charge[ftCalHit->m_channel.component];
			 //			cout << "charge = charge * cosmic_ene / mips_charge[ftCalHit->m_channel.component] " << charge << endl;
...
//			double timeWalkCorrection = -(tw[75][0] / (tw[75][1] + tw[75][2] * sqrt(faHit->m_charge)) + tw[75][3]);
			...
			/*			if (daq_gain[theCrate][faHit->m_channel.slot][faHit->m_channel.channel] < 0.01 && daq_gain[theCrate][faHit->m_channel.slot][faHit->m_channel.channel] > 0.3) {
			 cout << endl << "TW[" << ftCalHit->m_channel.component << "] " << this << " " << tw[ftCalHit->m_channel.component][0] << "  " << tw[ftCalHit->m_channel.component][1] << "  "
			 << tw[ftCalHit->m_channel.component][2] << "  " << tw[ftCalHit->m_channel.component][3] << endl;
			 cout << "Tempo non corretto " << 4. * faHit->m_time.count() << "  Correzione tw " << timeWalkCorrection << "  Tempo corretto " << 4 * faHit->m_time.count() + timeWalkCorrection
			 << endl;
			 cout << "Energia " << faHit->m_charge << " Energia corretta" << charge << " daq gain " << daq_gain[theCrate][faHit->m_channel.slot][faHit->m_channel.channel] << endl;
			 }*/
			...
			//Assign the uncorrected time to FTCalHit.
//			ftCalHit->setHitTime(4 * faHit->m_time.count());
			//Assign the crates delay corrected time
			/*			if (ftCalHit->m_channel.iX >= 12 || ftCalHit->m_channel.component == 224 || ftCalHit->m_channel.component == 242 || ftCalHit->m_channel.component == 360
			 || ftCalHit->m_channel.component == 445 || ftCalHit->m_channel.component == 96 || ftCalHit->m_channel.component == 142) {
