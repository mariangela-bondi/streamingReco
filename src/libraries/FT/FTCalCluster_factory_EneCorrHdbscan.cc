...
#include "FTCalCluster_factory_EneCorrHdbscan.h"
...
bool FTCalCluster_factory_EneCorrHdbscan::compareHits(const FTCalHit* a, const FTCalHit* b) {
	return (a->getHitEnergy() > b->getHitEnergy());
}

FTCalCluster_factory_EneCorrHdbscan::FTCalCluster_factory_EneCorrHdbscan() {
...
void FTCalCluster_factory_EneCorrHdbscan::CorrectClusterEnergy(FTCalCluster *clus) {
...
void FTCalCluster_factory_EneCorrHdbscan::Init() {

}

void FTCalCluster_factory_EneCorrHdbscan::ChangeRun(const std::shared_ptr<const JEvent> &aEvent) {
...
void FTCalCluster_factory_EneCorrHdbscan::Process(const std::shared_ptr<const JEvent> &aEvent) {

	//Loop su gli hits di un evento
	std::vector<const FTCalHitEneCorr*> hits = aEvent->Get<const FTCalHitEneCorr>();

	std::sort(hits.begin(), hits.end(), FTCalCluster_factory_EneCorrHdbscan::compareHits);
  ...
  float ene_threshold = 0.000; //GeV
	...
	//cout<<"::::: hdbscan with FTCalCluster_factory_EneCorrHdbscan"<<endl;
